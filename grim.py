...
from qgis.core import QgsRasterLayer, QgsVectorLayer, QgsMapLayerRegistry, QgsVectorFileWriter
...
class Grim:
    ...
    def run(self):
        ...
        self.copied_groyne_cell_polygon_path = None
        self.copied_groyne_cell_polygon = None
    ...
    def check_inputs(self):
        # Chec that all inputs are present. If so, allow the "Calculate" button to be pressed
        if self.results_directory_path is None or self.input_elevation_raster_path is None or self.input_groyne_cell_polygon_path is None:
        ...
        if self.input_elevation_raster_path is None:
            ...
            if self.input_elevation_raster_layer.isValid() is True:
                ...
                self.dlg.elevation_raster_check_label.setText("Digital elevation model has been selected but is not valid.")
                self.dlg.elevation_raster_check_label.setStyleSheet('color: red')
    ...
    def duplicate_input_groyne_cell_polygon(self):
        self.copied_groyne_cell_polygon_path = self.results_directory_path + "/copied_polygon.shp"
        QgsVectorFileWriter.writeAsVectorFormat(self.input_groyne_cell_polygon, self.copied_groyne_cell_polygon_path, "utf-8", None)
        self.copied_groyne_cell_polygon = QgsVectorLayer(self.copied_groyne_cell_polygon_path, "Copied", "ogr")
        
    def calculate_zonal_statistics(self):
        QgsMapLayerRegistry.instance().addMapLayer(self.input_elevation_raster_layer)
        QgsMapLayerRegistry.instance().addMapLayer(self.copied_groyne_cell_polygon)
        zonal_stats = QgsZonalStatistics(self.copied_groyne_cell_polygon, self.input_elevation_raster_path, "", 1,
    ...
    def select_mean(self):
        i = 0
        for feature in self.copied_groyne_cell_polygon.getFeatures():
            self.mean_elevations.append([i, feature['Mean']])
            i += 1
        self.create_csv()
    ...
    def create_csv(self):
        ...
        results_writer.writerow(["Groyne Cell", "Average Elevation"])
        for groyne_cell in self.mean_elevations:
